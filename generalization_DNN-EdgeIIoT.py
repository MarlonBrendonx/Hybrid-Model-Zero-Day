...
print("\nZero Day Classes Escolhidas")

for class_name in KNOW_CLASSES:
    print(f"=> {class_name}")


for classNumber in ZERO_DAY_CLASSES:
    print(f"{classNumber} => {le.classes_[classNumber]}")
...
for i, x_row in enumerate(X_train):
    ...
    if len(score_window) == WINDOW_SIZE and i % WINDOW_SIZE == 0:
